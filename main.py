...
def tfidf_recommend_titles(
    ...
    # 1. Keep calculations completely in sparse matrix format to save CPU/RAM
    qv = tfidf_matrix[idx]
    
    # 2. Perform dot product efficiently 
    similarity_sparse = tfidf_matrix.dot(qv.T)
    
    # 3. Only convert the final vector array to avoid memory freezes
    scores = similarity_sparse.toarray().ravel()

    # 4. Use partition instead of full sort to grab top items instantly
    if len(scores) > top_n + 1:
        # Gets the indices of top_n items without sorting the whole dataset
        raw_order = np.argpartition(-scores, top_n + 1)[:top_n + 1]
        order = raw_order[np.argsort(-scores[raw_order])]
    else:
