...
if st.session_state.view == "home":
    ...
    # HOME FEED MODE
   # HOME FEED MODE (Local offline fallback to bypass TMDB timeouts)
    st.markdown("### 🏠 Home — Local Dataset")
    
    # Safely query your local backend for a default movie title to pull initial data
    local_data, err = api_get_json("/recommend/tfidf", params={"title": "Avatar", "top_n": 24})
    
    if err or not local_data:
        st.error(f"Local dataset fallback failed: {err}")
        st.stop()
        
    # Build text-based display cards so the interface works without internet images
    fallback_cards = []
    for item in local_data:
        fallback_cards.append({
            "tmdb_id": 0,
            "title": item.get("title"),
            "poster_url": None  # Set to None since external posters are timing out
        })
        
    poster_grid(fallback_cards, cols=grid_cols, key_prefix="local_feed")
